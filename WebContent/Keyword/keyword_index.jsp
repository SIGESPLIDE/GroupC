			...
			<c:choose>
				...
				<c:when test="${empty keywords}">
					<div class="alert alert-info text-center py-5 shadow-sm"
						...
						<a
							href="${pageContext.request.contextPath}/keyword/keyword_register"
							class="btn btn-primary w-25 shadow-sm mb-1"> <i
							class="bi bi-plus-lg"></i> 新規登録
						</a>
					</div>
				</c:when>
				...
				<c:otherwise>
					...
					<div class="d-flex justify-content-end mb-3">
						<a
							href="${pageContext.request.contextPath}/keyword/keyword_register"
							class="btn btn-primary shadow-sm"> <i class="bi bi-plus-lg"></i>
							新規登録
						</a>
					</div>
							...
							<table class="table table-hover align-middle mb-0 text-center">
								...
								<tbody>
									<c:forEach var="k" items="${keywords}">
										<tr>
											<td class="text-start ps-4 fw-bold text-dark"><c:out
													value="${k.phrase}" /></td>
											<td>
												<button type="button"
													class="btn btn-outline-danger btn-sm px-4 btn-delete-trigger"
													data-bs-toggle="modal" data-bs-target="#deleteConfirmModal"
													data-id="${k.id}">削除</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
					...
					<%-- 削除用隠しフォーム --%>
					<form id="deleteForm"
						action="${pageContext.request.contextPath}/keyword/keyword_index"
						method="post">
