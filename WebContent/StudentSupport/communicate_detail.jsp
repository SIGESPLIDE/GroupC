...
<div
	...
	<div class="container mt-5 flex-grow-1 overflow-y-auto">
		...
		<div class="container w-100 h-75">
			<%-- 会話ボックス --%>
			<div class="w-auto h-75 border border-1 border-secondary rounded p-3"
					...
					<div class="m-2">
						<c:choose>
							...
							<c:when test="${chatLog.speaker==1}">
								<div class="text-end">
									<p class="mb-0">${chatLog.formattedDatetime}</p>
								</div>
								<div class="w-100 d-flex justify-content-end">
									<p
										class="m-2 mt-0 p-2 rounded bg-primary-subtle d-inline-block"
										style="max-width: 70%;">
										生徒:
										<c:out value="${chatLog.chatLog}" escapeXml="false" />
									</p>
								</div>
							</c:when>
							<%-- ユニボのふきだし --%>
							<c:otherwise>
								...
								<div class="w-100">
									<p
										class="m-2 mt-0 p-2 rounded bg-secondary-subtle d-inline-block"
										style="max-width: 70%;">
										ユニボ:
										<c:out value="${chatLog.chatLog}" escapeXml="false" />
									</p>
		...
		<div
			...
			<form
				...
				<input type="hidden" name="studentId"
					value="${chatLogList.get(0).studentId}">
