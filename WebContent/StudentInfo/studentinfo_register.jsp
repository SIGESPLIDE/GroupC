...
<%-- IDが重複した時に表示 --%>
        ...
        <form action="${pageContext.request.contextPath}/studentinfo/studentinfo_regist" method="post">
			<div class="container form-group form-control form-control-lg d-flex flex-column w-50">
					<label>生徒ID</label><input type="text" id="idInput" name="studentId" value="${studentId}" placeholder="生徒IDを入力してください" required>
					<label>年</label><input type="text" id="gradeInput" value="${grade}" name="grade" maxlength="4" placeholder="年を入力してください(4字以内)" required>
					<label>組</label><input type="text" id="claInput" value="${cla}" name="cla" maxlength="4" placeholder="組を入力してください(4字以内)" required>
					<label>名前</label><input type="text" id="nameInput" name="studentName" value="${studentName}" maxlength="25" placeholder="名前を入力してください(25字以内)" required>
			</div>
		</form>
